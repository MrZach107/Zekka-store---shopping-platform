...
const Men = () => {
  ...
  const derivedSizes = useMemo(() => {
    const base = allProducts.filter(p => (p.gender === 'men' || p.gender === 'unisex'));
    const sizes = new Set();
  ...
  return (
    <Container>
      <Announcement />
      <Navbar />
      <Title>{t('footer.manFashion')}</Title>
      <FilterContainer>
        <Filter>
      ...
      <Products category={undefined} filters={{ ...filters, gender: 'men' }} sort={sort} />
      <Footer />
    </Container>
  );
};
